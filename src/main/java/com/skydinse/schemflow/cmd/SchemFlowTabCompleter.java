...
/**
 * SchemFlowTabCompleter — tab completion for "SchemFlow" only.
 * Author: c4g7
 */
public class SchemFlowTabCompleter implements TabCompleter {
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String alias, String[] args) {
        ...
        // Suggest schematic names for paste/delete at arg #2
        if (args.length == 2 && ("paste".equalsIgnoreCase(args[0]) || "delete".equalsIgnoreCase(args[0]))) {
            String p = args[1].toLowerCase();
            java.util.List<String> cache = com.skydinse.schemflow.SchemFlowPlugin.getInstance().getSchematicCache();
            List<String> out = new ArrayList<>();
